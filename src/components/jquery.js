...
export function jQuery(a, c) {
  ...
  this.get(
    a.constructor == Array ||
      (a.length && !a.nodeType && a[0] != undefined && a[0].nodeType)
...
jQuery.fn = jQuery.prototype = {
  ...
  attr: function (key, value, type) {
    ...
    return key.constructor != String || value != undefined
      ? this.each(function () {
          ...
          else jQuery.attr(type ? this.style : this, key, value);
        })
      ...
      (t.constructor == Array &&
        jQuery.map(this, function (a) {
          for (var i = 0; i < t.length; i++)
            if (jQuery.filter(t[i], [a]).r.length) return a;
        })) ||
        (t.constructor == Boolean && (t ? this.get() : [])) ||
        (t.constructor == Function && jQuery.grep(this, t)) ||
  ...
  domManip: function (args, table, dir, fn) {
    ...
    return this.each(function () {
      ...
      for (
        var i = dir < 0 ? a.length - 1 : 0;
        i != (dir < 0 ? dir : a.length);
        i += dir
      ) {
...
jQuery.extend({
  init: function () {
    ...
    jQuery.each(jQuery.macros.axis, function (i, n) {
      jQuery.fn[i] = function (a) {
        ...
        if (a && a.constructor == String) ret = jQuery.filter(a, ret).r;
    ...
    jQuery.each(jQuery.macros.to, function (i, n) {
      jQuery.fn[i] = function () {
        var a = arguments;
        return this.each(function () {
          for (var j = 0; j < a.length; j++) $(a[j])[n](this);
        });
      };
    });
  ...
  find: function (t, context) {
    ...
    if (context && context.nodeType == undefined) context = null;
  ...
  sibling: function (elem, pos, not) {
    ...
    for (var i = 0; i < siblings.length; i++) {
      ...
      if (siblings[i].nodeType == 1) elems.push(siblings[i]);
      if (siblings[i] == elem) elems.n = elems.length - 1;
    }

    return jQuery.extend(elems, {
      last: elems.n == elems.length - 1,
      cur:
        (pos == "even" && elems.n % 2 == 0) ||
        (pos == "odd" && elems.n % 2) ||
        elems[pos] == elem,
  ...
  merge: function (first, second) {
    ...
    for (var k = 0; k < first.length; k++) result[k] = first[k];
  ...
  map: function (elems, fn) {
    // If a string is passed in for the function, make a function
    // for it (a handy shortcut)
    if (fn.constructor == String) fn = new Function("a", "return " + fn);
  ...
  event: {
    ...
    trigger: function (type, data, element) {
      ...
      if (!element) {
        ...
        if (g)
          for (var i = 0; i < g.length; i++) this.trigger(type, data, g[i]);
...
jQuery.macros = {
  ...
  each: {
    ...
    toggle: function () {
      $(this)[$(this).is(":hidden") ? "show" : "hide"].apply(
        $(this),
        arguments
      );
    },
    ...
    remove: function (a) {
      if (!a || jQuery.filter(a, [this]).r) this.parentNode.removeChild(this);
    },
    empty: function () {
...
jQuery.fn.extend({
  ...
  toggle: function (a, b) {
    ...
    // toggling on a click
    return a && b && a.constructor == Function && b.constructor == Function
...
jQuery.extend({
  setAuto: function (e, p) {
    ...
    if (
      (p == "height" && e.scrollHeight != o) ||
      (p == "width" && e.scrollWidth != o)
    )
      return;
  ...
  fx: function (elem, options, prop) {
    ...
    z.a = function () {
      if (options.step) options.step.apply(elem, [z.now]);
      ...
      } else if (parseInt(z.now)) y[prop] = parseInt(z.now) + "px";
    ...
    if (!z.el.oldOverlay) z.el.oldOverflow = jQuery.css(z.el, "overflow");
    ...
    z.step = function (firstNum, lastNum) {
      ...
      if (t > z.o.duration + z.startTime) {
        ...
        var done = true;
        for (var i in z.el.curAnim) if (z.el.curAnim[i] !== true) done = false;
...
jQuery.fn.load = function (url, params, callback, ifModified) {
  if (url.constructor == Function) return this.bind("load", url);
  ...
  jQuery.ajax(
    ...
    function (res, status) {
      ...
      } else callback.apply(self, [res.responseText, status]);
    },
    ifModified
  );
